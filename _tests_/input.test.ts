// TypeScript migrated test for input
...
describe('Input validation module', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (auth as any).allowedAccessTypes = ['jwt', 'aws_iam'];
  });

  test('valid input with all parameters', () => {
    jest.spyOn(core, 'getInput').mockImplementation((name: string) => {
      switch (name) {
        case 'access-id': return 'p-asdf';
        case 'access-type': return 'JWT';
        case 'api-url': return 'https://api.akeyless.io';
        case 'producer-for-aws-access': return '/path/to/aws/producer';
        case 'static-secrets': return '{"/some/static/secret":"secret_key"}';
        case 'dynamic-secrets': return '{"/some/dynamic/secret":"other_key"}';
    ...
    jest.spyOn(core, 'getBooleanInput').mockImplementation((name: string) => {
      switch (name) {
        case 'export-secrets-to-outputs': return true;
        case 'export-secrets-to-environment': return true;
        case 'parse-dynamic-secrets': return false;
