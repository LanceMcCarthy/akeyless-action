...
async function getAwsCloudIdV3(): Promise<string> {
  try {
...
async function jwtLogin(apiUrl: string, accessId: string) {
...
async function awsIamLogin(apiUrl: string, accessId: string) {
  const api = akeylessApi(apiUrl);
...
export const allowedAccessTypes = Object.keys(login);

export async function akeylessLogin(accessId: string, accessType: string, apiUrl: string) {
  try {
